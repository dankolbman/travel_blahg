...
@login_required
def home():
    p = Post.query.filter_by(author_id=current_user.id).order_by(
        desc(Post.timestamp)).limit(10).all()
...
def user_page(username):
    ...
    if user != None:
        p = Post.query.filter_by(author_id=user.id).order_by(
            desc(Post.timestamp)).limit(10).all()
        return render_template('index.html', posts=p, user=current_user, all_posts=Post.query.filter_by(author_id=current_user.id).all())
    else:
...
def prox(path):
    path = safe_join(current_app.config['UPLOAD_FOLDER'], path)
...
def ping_map():
    ...
    import json
    pings = Ping.query.filter_by(author_id=current_user.id).order_by(
        desc(Ping.timestamp)).limit(500).all()
...
def ping_map_user(username):
    ...
    pings = Ping.query.filter_by(author=user).order_by(
        desc(Ping.timestamp)).limit(500).all()
...
def new_image():
    ...
    if request.method == 'POST':
        ...
        if f and allowed_file(f.filename):
            ...
            large = os.path.join(
                'blog', current_app.config['UPLOAD_FOLDER'],  lfname)
            im.save(large)

            im.thumbnail((512, 512))
            med = os.path.join(
                'blog', current_app.config['UPLOAD_FOLDER'], mfname)
            im.save(med)

            im.thumbnail((128, 128))
            small = os.path.join(
                'blog', current_app.config['UPLOAD_FOLDER'], sfname)
            im.save(small)
